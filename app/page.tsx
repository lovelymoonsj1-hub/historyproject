...
const referenceVisuals: ReferenceVisual[] = [
  ...
  { terms: ["\ubabd\uace8\uc758 \uce68\uc785", "\uc0bc\ubcc4\ucd08"], src: "/images/reference/ybm-history-44.jpg", title: "\ubabd\uace8\uacfc \uace0\ub824", note: "\ubabd\uace8\uc758 \uce68\uc785, \uac15\ud654\ub3c4 \ucc9c\ub3c4, \uc0bc\ubcc4\ucd08\uc758 \ud56d\uc7c1\uc744 \ud558\ub098\uc758 \ud750\ub984\uc73c\ub85c \uc774\ud574\ud574 \ubcf4\uc138\uc694." },
...
function LandingHero({ onStart, canStart }: { onStart: () => void; canStart: boolean }) {
      ...
      <div className="grid items-center gap-5 p-5 md:grid-cols-[.78fr_1.22fr] md:p-9">
        <div className="relative z-10 px-2 py-5 md:px-5">
          ...
          <p className="mt-6 max-w-md text-lg leading-8 text-stone-600 md:text-xl md:leading-9">인물, 사건, 문화유산을 시간의 흐름으로 연결하며 우리 역사를 탐험해요.</p>
           <button type="button" onClick={onStart} disabled={!canStart} className="mt-7 rounded-full bg-[#57958f] px-7 py-3.5 text-base font-black text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-[#397e79] disabled:cursor-not-allowed disabled:opacity-60 md:text-lg">{canStart ? "시간 여행 시작하기 →" : "로그인 후 이용할 수 있어요"}</button>
           {!canStart && <p className="mt-3 text-sm font-bold text-[#b36b2c]">오른쪽 위에서 학습 ID로 로그인하거나 계정을 만들어 주세요.</p>}
  ...
  return <section className="mx-auto max-w-6xl px-5 py-8 md:px-10 md:py-12" aria-label="로그인 후 역사 탐색 홈">
    <div className="relative overflow-visible rounded-[34px] border border-[#ead0aa] bg-[#fff9ef] shadow-sm">
      ...
      <div className="relative mx-auto flex min-h-[520px] max-w-4xl flex-col items-center justify-center px-5 py-16 text-center md:min-h-[570px]">
        ...
        {query && visibleSuggestions.length > 0 && <div className="absolute left-0 right-0 top-[calc(100%+0.6rem)] z-20 overflow-hidden rounded-2xl border border-[#e0c9a8] bg-white p-2 text-left shadow-xl" role="listbox" aria-label="검색어 자동완성">{visibleSuggestions.map((entry)=><button type="button" role="option" key={`home-suggest-${entry.title}`} onClick={()=>onSelect ? onSelect(entry) : onStart()} className="flex w-full items-center gap-3 rounded-xl px-4 py-3 hover:bg-[#fff3df]"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#eaf5ed] text-[#57958f]">↗</span><span><b className="block text-sm">{entry.title}</b><small className="block text-xs text-stone-500">{entry.era} · {entry.type}</small></span></button>)}</div>}
        </div>
...
export default function Home() {
  ...
  // 홈 화면 자동완성은 중앙 검색창에서만 표시하고, 상단 검색창용 목록은 상세 화면에서만 사용합니다.
  const suggestions = showWelcome ? [] : results.slice(0, 5);
  const homeSuggestions = results.slice(0, 5);
  ...
  useEffect(() => {
    if (!showWelcome && query.trim() && results[0]) {
      setSelectedId(results[0].title);
      setQuery("");
    }
  }, [showWelcome]);
  ...
  return <main className="min-h-screen bg-[#fbf3e4] text-[#41382e]">
    ...
    <header className="sticky top-0 z-30 flex flex-wrap items-center justify-between gap-3 border-b border-[#ead5b8] bg-[#fff9ef]/95 px-5 py-3 backdrop-blur md:px-12"><div><button type="button" onClick={goHome} className="text-left" aria-label="첫 화면으로 돌아가기"><b className="text-2xl tracking-tight text-[#d2744d]">{t.app}</b><span className="ml-2 text-xs text-stone-500">{t.sub}</span></button></div><div className="flex items-center gap-2">{!showWelcome && <label className="relative w-[min(47vw,440px)]"><span className="absolute left-3 top-2 text-lg text-[#57958f]">⌕</span><input className="w-full rounded-full border border-[#e0c9a8] bg-white px-9 py-2 text-sm outline-none focus:border-[#57958f]" value={query} onChange={(event)=>setQuery(event.target.value)} onKeyDown={(event)=>{ if (event.key === "Enter" && query.trim()) setShowWelcome(false); }} placeholder={t.search} aria-label={t.search}/></label>}<LearningAccount /></div></header>
     {showWelcome ? (hasSession ? <LoggedInHome query={query} onQueryChange={setQuery} onStart={() => setShowWelcome(false)} /> : <LandingHero canStart={false} onStart={() => undefined} />) : <section className="mx-auto max-w-6xl px-5 py-9 md:px-10"><p className="text-xs font-bold text-[#d2744d]">{t.home}</p><h1 className="mt-2 text-4xl font-black tracking-tight md:text-5xl">{selected.title}</h1><p className="mt-2 text-stone-500">{selected.era} · {selected.years}</p>
      {query && <section className="mt-5 rounded-3xl border border-[#e6d0b1] bg-white p-5"><h2 className="font-bold">{t.result} <span className="text-[#d2744d]">{results.length}</span></h2>{results.length ? <div className="mt-3 grid gap-2 md:grid-cols-2">{results.map((entry)=><button className="rounded-2xl bg-[#fff5e5] p-4 text-left hover:bg-[#e8f3ed]" key={entry.title} onClick={()=>select(entry)}><small className="text-[#57958f]">{entry.era} · {entry.type}</small><b className="ml-2">{entry.title}</b><p className="mt-2 text-xs text-stone-600">{entry.summary}</p></button>)}</div> : <p className="mt-3 text-sm text-stone-600">{t.noResult} {t.hint}</p>}</section>}
       <div className="mt-7 grid gap-5 lg:grid-cols-[1.45fr_.8fr]">
        ...
        <aside><GeminiSearchPanel /><div className="mt-5 rounded-[28px] border border-[#e7d0af] bg-white p-5"><h2 className="text-lg font-black">{t.related}</h2><div className="mt-3 divide-y divide-[#efe1cc]">{selected.related.map((item)=><button type="button" className="flex w-full items-center gap-3 py-4 text-left hover:text-[#57958f]" key={`${item.label}-${item.kind}`} onClick={()=>follow(item.label)} aria-label={`${item.label} 개념 보기`}><span className="grid h-8 w-8 place-items-center rounded-full bg-[#f9e6c6] text-xs">↗</span><span><small className="block text-xs text-stone-500">{item.kind}</small><b>{item.label}</b></span></button>)}</div><div className="mt-5 rounded-2xl bg-[#fff0cf] p-4"><small className="font-bold text-[#aa6d28]">{t.prompt}</small><p className="mt-1 text-sm leading-6">{selected.prompt}</p></div></div></aside>
      </div>
