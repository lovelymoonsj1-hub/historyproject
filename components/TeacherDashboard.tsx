...
export default function TeacherDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const student = new Map<string, { attempts: number; correct: number }>();
    records.forEach((record) => {
      ...
      const s = student.get(record.user_id) ?? { attempts: 0, correct: 0 };
    ...
    const students = members.map((member) => {
      const value = student.get(member.user_id) ?? { attempts: 0, correct: 0 };
